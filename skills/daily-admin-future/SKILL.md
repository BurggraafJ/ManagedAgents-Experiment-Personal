...
description: "Future-variant van Daily Admin (display-naam 'Administratie · Toekomst'). Scant ALLE aankomende externe-attendee events in Outlook-agenda (28d vooruit), classificeert per categorie (recruitment / customer / sales / lead / partner / onbekend) via Jira-REC + HubSpot-mirror + partner_domains, en doet voor onbekend een RAG-lookup via context-build. Sales/leads-flow schrijft alleen ECHTE twijfelgevallen als voorstel naar agent_proposals — al-lopende relaties (Customer Base, Sales-deal voorbij kennismaking, personal domain, lead met al-ingeplande historie) worden geskipt. **Recruitment-events landen sinds v1.16 in een aparte tabel `recruitment_meetings`** (geen voorstel-card, eigen sectie in Toekomst-tab) — zo blijft de kennismakingen-bucket puur sales/leads. Voert nooit direct mutaties door. Los van daily-admin zodat huidig-flow ongewijzigd blijft. Trigger op 'scan toekomst', 'future scan', 'kennismakingen voorbereiden', of dagelijks 07:00 NL via orchestrator."
---

# daily-admin-future — v1.16

> **v1.16 (2026-05-06):** Recruitment-flow gesplitst van sales/leads. Recruitment-events schrijven niet meer als `agent_proposals` (kandidaat-voorstel-cards in Goedkeuren). In plaats daarvan upsertet de skill ze in `recruitment_meetings` — eigen sectie in de Toekomst-tab, los van de "Eerste kennismakingen"-tabel. Bestaande pending recruitment-voorstellen zijn bij de migratie geclosed. Counts: `recruitment_meetings_recorded` / `recruitment_meetings_failed`.
