...
| 2026-05-04 | datascience skill (Jelle iteratie 18 — JelleMind project-replanning) | **JelleMind-rollout opgesplitst tot self-contained losse-sessie-project.** Inventarisatie: 80 signalen, 3 pending proposals (sinds 30 april), **0 actieve lessons** = blokkade voor consumer-rollout. Touchpoints met R.6 CaaS in kaart: lessons kunnen via `context-build` als 4e laag (kennislaag in bundle) — één wijziging dekt alle 6 consumer-skills ipv 6× per-skill werk. Dat is dramatisch simpeler dan oude F.3+F.4+F.6 plan in Intelligence Engineering. **Confluence**: (a) [Intelligence Engineering 422084620](https://bg-intelligence.atlassian.net/wiki/spaces/LM/pages/422084620) gemarkeerd als afgerond — F.1/F.2/F.5/F.6/F.7 al afgevangen door RAG-roadmap, F.3/F.4 verschoven; verplaatst naar Afgerond folder 414711809. (b) Nieuwe **[Project — JelleMind Activation 426377220](https://bg-intelligence.atlassian.net/wiki/spaces/LM/pages/426377220)** onder Lopende projecten, self-contained voor losse sessie. Beschrijft 5 fases: Phase A (Jelle reviewt 3 pending → ≥1 lesson), Phase B (schema: + 3 kolommen op `context_intents` + 1 op `context_bundles`), Phase C (`context-build` v1.2 met optionele lesson-injection per intent), Phase D (6 SKILL.md docs-update voor lesson-consumeren in prompt), Phase E (R.7 quality-loop pakt automatisch op). Per intent specifieke mind_scope-mix gedefinieerd (draft_reply: jelle+skill, enrich_record: skill+legalmind, etc.). Beslissingen vastgepind: min_similarity 0.40 (hoger dan chunks ivm prompt-bloat-risk), soft-fail bij match-error, search/learn_pattern injecteren NIET. Verifieer-snippets klaar. Schatting: ~3-4u Claude-werk + 10 min Jelle-review. Cost ~cents/maand, rollback = `inject_jellemind=false` in context_intents. **Niet uitgevoerd in deze sessie** — Phase A wacht op Jelle's review actie. |
| 2026-05-04 | datascience skill (Jelle iteratie 19 — BM25-fix in match_chunks) | **BM25 toonde structureel 0% in zoekpagina** — twee onafhankelijke oorzaken gefixt in één RPC-update. (1) **Schaal-mismatch**: `ts_rank_cd` levert raw waardes 0.0005–0.30; vector `1 - cosine` levert 0.5–0.95. UI vermenigvuldigt beide met 100 → vector toont 50–95%, BM25 toont 0–30% (mediaan rondt naar 0%). Fix in `match_chunks`: per-query max-norm via window-functie `bm25_raw / greatest(max(bm25_raw) OVER (), 0.05)`, geclampt op 1.0. Floor 0.05 voorkomt dat een query met alleen zwakke hits visueel wordt opgeblazen. **Ranking blijft identiek** — combined_score gebruikt RRF op ranks, niet op raw scores, dus norm raakt alleen de display. (2) **AND-modus stopt multi-word queries**: `plainto_tsquery('dutch', q)` produceert AND-tsquery (`besprak & recent & wintertal`), waardoor natuurlijke-taal-vragen vrijwel altijd 0 BM25-hits gaven. Fix: vervangen door `to_tsquery('dutch', regexp_replace(plainto_tsquery::text, ' & ', ' | '))` — OR-tsquery zodat docs met deelmatches ook hits worden. NULLIF-wrapper handelt stopwoorden-only edge case (geen hits). `ts_rank_cd` zorgt zelf dat docs met meer matchende termen hoger scoren. **Migrations**: `match_chunks_bm25_per_query_normalize_2026_05_04` + `match_chunks_bm25_or_tsquery_2026_05_04`. Smoke-test "wat besprak ik recent met Wintertaling": top-10 nu gemengd vector + BM25, top BM25-norm 0.276 (was 0.000), pure-vector hits behouden hun rangschikking. Geen Edge Function of frontend wijzigingen nodig — de RPC-output `out_bm25_score` is hetzelfde veld dat de UI al consumeert. |
| 2026-05-04 | datascience skill (Jelle iteratie 20 — context-build auth + hybrid-mix + Grok-prefix display + UI-witregels) | **Vier verbeteringen op zoekpagina + retrieval-stack na Jelle's vervolgvragen.** (1) **`context-build` 401-bug**: function was met `verify_jwt:true` gedeployed in v3, dus Supabase-gateway weigerde alle calls van rag-search/autodraft-rag-prefill (die cron_secret als bearer sturen, geen JWT). Fix: redeploy v6 met `verify_jwt:false` + expliciete Bearer-check op `cron_secret` in code (defense-in-depth, consistent met andere interne edge functions). 401's stoppen direct, autodraft-rag-prefill 546-timeouts ook (was hammering context-build in loop). (2) **Hybrid-mix probleem**: chunks die alleen in top-100 vector zaten kregen bm25_score=0 (en vice versa), waardoor je nooit een rij met BEIDE scores zag. Fix in `match_chunks` v3 + `match_chunks_for_entity` v4: structuur herschreven naar `candidates → fused_ranks → enriched`-flow. In `enriched`-CTE wordt voor ELKE candidate-chunk via JOIN op chunks-tabel zowel vector_score (cosine) als bm25_raw (ts_rank_cd) berekend, ongeacht in welke top-lijst hij oorspronkelijk zat. RRF blijft op ranks (originele top-100 lijsten), display krijgt nu mooie hybrid-mix. Smoke-test "Wintertaling offerte proefperiode" toont nu rijen met `vec=0.72 + bm25=0.82` op zelfde regel. (3) **Grok-prefix display**: `chunks.content_with_context` (door GPT-5-nano augmented) werd niet door RPC's gereturned. Beide RPC's nu uitgebreid met `out_content_with_context` kolom (DROP+CREATE — return-type change). Edge Functions doorgepiped: context-build v6 mappet naar `content_with_context` veld in `matches[]`, rag-search v7 forward naar response. (4) **UI-witregels + prefix-block in `RagSearchView.jsx`**: oude `cleanText` deed `.replace(/\s+/g, ' ')` — collapseerde alle whitespace incl. newlines. Vervangen door HTML→newline-conversie (`<br>`/`</p>`/`</div>` → `\n`) gevolgd door alleen-horizontale-whitespace-collapse. Nieuwe `splitAugmented(content_with_context, content)` splitst op eerste `\n\n` → prefix wordt apart gerenderd in italic + paarse links-border + lichte paarse achtergrond met tooltip "Contextuele zin toegevoegd door de chunker (GPT-5-nano), gebruikt voor embedding + BM25". Body-content rendert nu netjes met paragraaf-breaks. Recency_score in beide RPC's geclampt op 1.0 (toekomstige events kregen >1.0 wat onlogisch is voor "recency"). **Bonus**: gedurende deze ronde ontdekt dat `chunks` ook al sinds R.3 een `content_with_context`-kolom had — geen nieuwe migratie of backfill nodig, alleen exposeren. |
