 ...
 *  - linkedinDrafts    (legal_ai_linkedin_posts, laatste 10)
 *
 * Plus drie write-acties: submitFeedback, requestLinkedInDraft, decideProposal.
 *
...
export function useLegalAI(activeTrack) {
  ...
  const requestLinkedInDraft = useCallback(async (articleId) => {
    ...
    const { data, error: err } = await supabase.from('legal_ai_skill_requests').insert({
