...
// PlatformPage (admin) — merged view van Configuratie + Edge Functions +
// Database volgens "option B": cockpit met KPIs + drie segmenten (Config /
// Edge / Sync). Maestro Organisatie-redesign, v1.167 (2026-09-12).

const REFRESH_MS = 30_000

// === Config Items (from ConfiguratiePage) ===
const CONFIG_ITEMS = [
  {
...
// === Edge Functions (from EdgeFunctionsPage) ===
const FUNCTIONS = [
...
export default function PlatformPage() {
  ...
  return (
    <>
      {/* Cockpit: KPIs at the top */}
      <div className="admin-cockpit">
        <div className="admin-cockpit__card">
          <div className="admin-cockpit__label">Edge Functions</div>
          <div className="admin-cockpit__value">
            <span className="admin-pill admin-pill--ok"><span className="admin-pill__dot" />{okCount} ok</span>
            {errCount > 0 && (
              <span className="admin-pill admin-pill--err" style={{ marginLeft: 8 }}><span className="admin-pill__dot" />{errCount} fout</span>
            )}
          </div>
        </div>
        <div className="admin-cockpit__card">
          <div className="admin-cockpit__label">Platform Status</div>
          <div className="admin-cockpit__value">
            <span className="admin-pill admin-pill--ok"><span className="admin-pill__dot" />Operationeel</span>
          </div>
        </div>
        <div className="admin-cockpit__card">
          <div className="admin-cockpit__label">Laatst ververst</div>
          <div className="admin-cockpit__value" style={{ fontSize: 14, color: 'var(--text-muted)' }}>
            {fetchedAt ? fetchedAt.toLocaleTimeString('nl-NL', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '—'}
          </div>
        </div>
      </div>

      {/* Segment switcher */}
      <div className="admin-segment-bar">
        <button
          type="button"
          className={`admin-segment-btn ${segment === 'config' ? 'is-active' : ''}`}
          onClick={() => setSegment('config')}
        >
          Configuratie
        </button>
        <button
          type="button"
          className={`admin-segment-btn ${segment === 'edge' ? 'is-active' : ''}`}
          onClick={() => setSegment('edge')}
        >
          Edge Functions
        </button>
        <button
          type="button"
          className={`admin-segment-btn ${segment === 'sync' ? 'is-active' : ''}`}
          onClick={() => setSegment('sync')}
        >
          Database Sync
        </button>
      </div>
      ...
      {/* Segment content */}
      <div className="admin-segment-content">
        {segment === 'config' && (
