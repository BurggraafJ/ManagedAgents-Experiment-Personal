 ...
 * (Refresh, Voice, Nieuw event zitten in agm-topbar). Pill-toggles met dot,
 * weeklabel met info-line, geen build-tag in toolbar.
 *
 * Alle state-handlers worden ongewijzigd doorgegeven uit AgendaMaestroView →
 * useAgenda + useAgendaDerived blijven autoritair voor data.
 */
export default function AgendaMaestroToolbar({
  weekStart,
  onPrev,
  onNext,
  onToday,
  ...
  return (
    <div className="agm-toolbar">
      <div className="agm-toolbar__nav">
        <button type="button" className="agm-btn agm-btn--sm" onClick={onToday} title="Naar deze week">Vandaag</button>
        <button type="button" className="agm-arrow" onClick={onPrev} aria-label="Vorige week">
        ...
        <button type="button" className="agm-arrow" onClick={onNext} aria-label="Volgende week">
      ...
      <div className="agm-week-label">
        ...
        <small>
          ...
          {proposalsOpenCount > 0 && (
            <> · <span className="agm-week-label__accent">{proposalsOpenCount} voorstellen open</span></>
          )}
        </small>
      </div>

      <div className="agm-toolbar__actions">
        <button
          type="button"
          className={`agm-toggle ${showRules ? 'is-active' : ''}`}
          onClick={onToggleRules}
          title="Toon shadow-blokken: reistijd, verkeer, interne dag"
          aria-pressed={showRules}
        >
          <span className="agm-toggle__dot" />
          Regels
        </button>
        <button
          type="button"
          className={`agm-toggle ${showProposals ? 'is-active' : ''}`}
        ...
        >
          ...
          Voorstellen
          {proposalsOpenCount > 0 && (
            <span className="agm-toggle__count">{proposalsOpenCount}</span>
          )}
        </button>
        {proposalsOpenCount > 0 && (
          <button
          ...
          {days.map(d => {
            ...
            return (
              <button
                key={d.toISOString()}
                type="button"
                className={`agm-daybtn ${active ? 'is-active' : ''} ${isToday ? 'is-today' : ''}`}
                onClick={() => onSelectDay(d)}
              >
                <span className="agm-daybtn__dow">{dow}</span>
                <span className="agm-daybtn__num">{date}</span>
              </button>
            )
          })}
        </div>
