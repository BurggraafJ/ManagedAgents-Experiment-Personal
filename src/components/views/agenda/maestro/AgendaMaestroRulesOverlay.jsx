...
import { AGM_HOUR_HEIGHT } from './AgendaMaestroEventCard'

/* AgendaMaestroRulesOverlay — shadow-laag voor één agm-grid__daycol.
 * Functioneel gelijk aan AgendaRulesOverlay (zelfde rule-types, prioriteiten,
 * merge-blocks), maar emit alleen agm-* class-namen en 48px hour-height. */
export default function AgendaMaestroRulesOverlay({
  day,
  events,
  rules,
  showRules,
  ...
  if (showRules) {
    if (beforeRule) {
      shadowBlocks.push({
        ...
        className: 'agm-shadow--before9', label: 'Geen meetings',
    ...
    if (showTrafficMorning) {
      shadowBlocks.push({
        ...
        className: 'agm-shadow--traffic', label: 'Verkeer',
    ...
    if (eveningRule) {
      shadowBlocks.push({
        ...
        className: 'agm-shadow--evening', label: `Geen meetings na ${eveningRule.params.block_start}`,
    ...
    if (travelBufferRule) {
      for (const { ev, classified } of timed) {
        ...
        shadowBlocks.push({
          ...
          className: 'agm-shadow--travel', label: 'Reistijd · 30m',
          priority: travelBufferRule.priority || 100, idx: _idx++,
        })
        shadowBlocks.push({
          startMin: endMin, endMin: endMin + 60,
          className: 'agm-shadow--travel', label: 'Reistijd',
          priority: travelBufferRule.priority || 100, idx: _idx++,
        })
      }
    }
    if (postBufferRule) {
      for (const { ev } of timed) {
        ...
        shadowBlocks.push({
          ...
          className: 'agm-shadow--postbuffer', label: 'Speling',
  ...
  return (
    <>
      {showRules && wednesdayRule && isWednesday && (
        <div
          className="agm-shadow agm-shadow--internal-day"
          style={{ top: 0, height: `${HOURS * AGM_HOUR_HEIGHT}px` }}
      ...
      {showProposals && proposals.map(({ slot, proposal }, i) => {
        ...
        const top    = Math.max(0, (startMin / 60) * AGM_HOUR_HEIGHT)
        const height = Math.max(20, ((endMin - startMin) / 60) * AGM_HOUR_HEIGHT)
        return (
          <div
            key={`prop-${proposal.id}-${i}`}
            className="agm-proposal"
            style={{ top: `${top}px`, height: `${height}px` }}
            title={`Voorgesteld aan ${proposal.recipient_name || proposal.recipient_email || 'onbekend'} — ${proposal.subject_context || ''}`}
          >
            <span className="agm-proposal__title">
              ⊕ Voorgesteld · {proposal.recipient_name || proposal.recipient_email || 'voorstel'}
            </span>
            {proposal.subject_context && (
              <span className="agm-proposal__sub">{proposal.subject_context}</span>
            )}
          </div>
        )
      })}
...
function ShadowBlock({ startMin, endMin, className, label }) {
  ...
  const height = ((endMin - startMin) / 60) * AGM_HOUR_HEIGHT
  if (height <= 0) return null
  return (
    <div className={`agm-shadow ${className}`} style={{ top: `${top}px`, height: `${height}px` }} title={label}>
      {height > 18 && <span className="agm-shadow__label">{label}</span>}
    </div>
  )
}
