                ...
                <div style={{
                  ...
                  {m.error && (
                    <span style={{ color: '#ef4444' }}>
                      Fout: {m.error}
                      {m.error.includes('anthropic_api_key_missing') && (
                        <span style={{ display: 'block', marginTop: 6, fontSize: 11 }}>
                          Voeg een Anthropic-key toe via <Link to="/instellingen">Instellingen → Skill Credentials</Link> onder key <code>skill:anthropic:api_key</code>.
                        </span>
                      )}
                    </span>
                  )}
                  ...
                  {!m.error && !m.loading && m.role === 'assistant' && m.tokens && (
                    <div style={{ marginTop: 8, fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
                      {m.timing_ms?.total}ms · {m.citations?.length || 0} bronnen · {m.tokens.chat_in + m.tokens.chat_out} tokens
                    </div>
                  )}
                </div>
              </div>
