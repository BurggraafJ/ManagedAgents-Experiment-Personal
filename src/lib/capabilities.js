...
// ── Bundels ────────────────────────────────────────────────────────────────
// Beslissing 3 (Jelle, 2026-09-14): Organisatie is **één vinkje**. De database
// houdt de vijf rechten los — zeven vinkjes later is dan een UI-wijziging en
// geen migratie — en deze laag zet ze samen via `capabilities.ui_bundle`.
const BUNDLE_META = {
  organisatie: {
    label: 'Organisatie',
    omschrijving: 'Gebruikers · Health · Security · Skills · Pijplijn — samen één vinkje.',
  },
}
...
export function buildRows(caps) {
  ...
  for (const cap of caps) {
    ...
    rows.push({
      ...
      toelichting: leden.find(c => c.toelichting)?.toelichting || null,
      bundel: true,
    })
  }
  return rows
...
// Telt per losse capability, niet per rij — de bundel is een UI-vorm, de
// handhaving blijft per recht.
export function userStatsFor(caps, role, overrides, presetKeys) {
  let aan = 0
  let afwijkend = 0
  for (const cap of caps) {
    const ov = overrides.get(cap.key)
    if (isEffective(cap, role, ov, presetKeys)) aan++
    // Afwijkend = de override zegt écht iets anders dan de preset. Een
    // 'grant' op iets dat de rol al geeft is geen afwijking, en een override
    // op een niet-uitdeelbaar recht telt nooit mee — has_capability negeert
    // hem ook.
    if (ov && cap.grantable && (ov.effect === 'grant') !== presetKeys.has(cap.key)) {
      afwijkend++
    }
  }
  return { aan, afwijkend, totaal: caps.length }
}
