...
**Auth:** voor Supabase DB-toegang (RPC's voor suggest/detect, UPDATE op `tasks`) en voor de `task-organizer-fireflies` Edge Function: zie [`agent-handbook/references/authentication.md`](../agent-handbook/references/authentication.md). DB-only skill — geen Composio/externe API. Edge Function gebruikt service_role automatisch; Claude-sessies gebruiken Supabase MCP of `vault-read-proxy` voor cron_secret.
