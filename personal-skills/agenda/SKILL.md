...
**Auth:** voor Supabase DB-toegang (lezen calendar/forecast/rules/voice-notes, schrijven `agenda_*` tabellen): zie [`agent-handbook/references/authentication.md`](../agent-handbook/references/authentication.md). DB-only skill — geen Composio/externe API. Edge Function gebruikt service_role automatisch; Claude-sessies gebruiken Supabase MCP of `vault-read-proxy` voor cron_secret.
