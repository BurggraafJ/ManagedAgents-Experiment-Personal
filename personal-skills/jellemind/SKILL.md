...
**Auth:** voor Supabase DB-toegang (RPC's, agent_runs writes) en voor de manuele `jellemind-embed` Edge Function trigger met `cron_secret`: zie [`agent-handbook/references/authentication.md`](../agent-handbook/references/authentication.md). DB-only skill — geen Composio/externe API. Edge Function gebruikt service_role automatisch; Claude-sessies gebruiken Supabase MCP of `vault-read-proxy`.
